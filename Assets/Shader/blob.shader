...
float scene(float3 position){
    ...
    position /=2;
    sphere += (sin(20*position.x)*sin(20*position.y * (_CosTime.r-1.5)*0.6*sin(20*position.z +((_SinTime.g-1.5)*0.2) )))*0.1;
...
float4 trace (float3 origin, float3 direction){
    ...
    for(int i = 0 ; i < steps; i++){
        ...
        if (dist < smallNumber){
		    ...
		    fixed3 K_s = fixed3(1.4, 0.4, 1.2);
		    float shininess = 1.0;
		    fixed3 color = phongIllumination(K_a, K_d, K_s, shininess, positionOnRay, origin, _Time.z * 6) ;
            fixed4 colFromPic = tex2D (_MainTex, positionOnRay.xy);
            return float4(colFromPic.r,color.g,color.b,a);
