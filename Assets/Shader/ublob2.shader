...
float scene(float3 position){
    ...
    sphere -= (sin(20*position.x)*sin(20*position.y * (_CosTime.r-1.5)*0.6*sin(20*position.z +((_SinTime.g-1.5)*0.2) )))*0.5;
...
float4 trace (float3 origin, float3 direction){
    ...
    for(int i = 0 ; i < steps; i++){
        ...
        if (dist < smallNumber){
            ...
            fixed3 K_a = fixed3(0.5, 0.5, 0.5); //  _LightColor0.rgb;// //ambient
		    fixed3 K_d =fixed3((sin(a*30)+1) /2,0,cos(a)+1 /2);
		    fixed3 K_s = fixed3(1.10, .1, (positionOnRay.y%2.0));
		    float shininess = 1.0;
		    fixed3 color = phongIllumination(K_a, K_d, K_s, shininess, positionOnRay, origin, _Time.z * 5) ;
